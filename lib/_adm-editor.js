...
module.exports = {
        ...
        fs.writeFile(
            ...
            function(err) {
                ...
                if (req.body.fileName == req.params.file) {
                   ...
                   res.redirect('/' + Options.adminDir + '/' + req.params.contentType + '?' + req._admin.name + '&' + req._admin.pass);
                } else {
                    fs.rename(
                        req.params.contentType + category + '/' + req.params.file,
                        ...
                        function(err) {
                            if ( err ) {
                                ...
                                // _this.editFile(req, res, 'Done!');
                                res.redirect('/_adm/' + req.params.contentType + '?' + req._admin.name + '&' + req._admin.pass);
                            }
                        }
                    );
                }
    ...
    _getContentHTML: function (req, subDir) {
        ...
        _.forEach(
            ...
            function (fileName) {
                subDirHTML +=
                    '<li>' +
                        '<a href="/' + Options.adminDir + '/' + req.params.contentType + subDirName + '/' + fileName +
    ...
    addCategory: function (req, res) {
        ...
        res.redirect('/' + Options.adminDir + '/content' + '?' + req._admin.name + '&' + req._admin.pass);
    }
